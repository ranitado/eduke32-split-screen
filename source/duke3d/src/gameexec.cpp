...
static GAMEEXEC_INLINE int32_t G_ClassicWeaponStayEnabled(void)
{
...
static void P_AddWeaponMaybeSwitch(DukePlayer_t * const ps, int const weaponNum)
{
    ...
    int const weaponSwitch = ps->weaponswitch;

    if ((weaponSwitch & (1|4)) == (1|4))
    {
        int       new_wchoice  = -1;
        int       curr_wchoice = -1;
        ...
        {
            int w = G_GetLocalPlayerWeaponChoiceSetting(playerNum, i);
    ...
    {
        P_AddWeapon(ps, weaponNum, (weaponSwitch & 1));
    }
}
...
static int G_GetShowViewPlayerNum(vec3_t const &vec, int const sect)
{
    int playerNum = ((unsigned)vm.playerNum < MAXPLAYERS && g_player[vm.playerNum].ps != nullptr) ? vm.playerNum : screenpeek;
    ...
    if (G_HaveSplitScreen())
    {
        ...
        {
            int const splitPlayerNum = G_GetSplitScreenPlayer(viewIndex);
            if ((unsigned)splitPlayerNum >= MAXPLAYERS)
                continue;

            auto const pPlayer = g_player[splitPlayerNum].ps;
            if (pPlayer == nullptr)
                continue;

            if (pPlayer->cursectnum == sect && pPlayer->pos.x == vec.x && pPlayer->pos.y == vec.y)
                return splitPlayerNum;
        }
    }
#endif
...
static void G_ShowView(vec3_t vec, fix16_t a, fix16_t horiz, int sect, int ix1, int iy1, int ix2, int iy2, int unbiasedp)
{
    ...
    int const viewPlayer   = G_GetShowViewPlayerNum(vec, sect);
    int const savedPeek    = screenpeek;
    int const savedMirror  = display_mirror;
    auto const savedPos    = CAMERA(pos);
    auto const savedAng    = CAMERA(q16ang);
    auto const savedHoriz  = CAMERA(q16horiz);
    int16_t const savedSect = CAMERA(sect);

    screenpeek      = viewPlayer;
    CAMERA(pos)     = vec;
    CAMERA(q16ang)  = a;
    CAMERA(q16horiz)= horiz;
    CAMERA(sect)    = sect;
    ...
    display_mirror = savedMirror ? savedMirror : 2;
    G_DoSpriteAnimations(vec.x, vec.y, vec.z, fix16_to_int(a), smoothratio);
    display_mirror = savedMirror;
    renderDrawMasks();
    G_RestoreInterpolations();
    CAMERA(pos)      = savedPos;
    CAMERA(q16ang)   = savedAng;
    CAMERA(q16horiz) = savedHoriz;
    CAMERA(sect)     = savedSect;
...
//
            ...
            vInstruction(CON_IFGOTWEAPONCE):
                ...
                if (G_LocalPerPlayerPickupsEnabled() && vm.pSprite->owner == vm.spriteNum && vm.playerNum >= 0
                    && G_IsLocalPerPlayerPickup(vm.pSprite->picnum))
                {
                    if (*insptr == 0)
                    {
                        branch(G_PlayerHasLocalPerPlayerPickup(vm.playerNum, vm.spriteNum));
                        dispatch();
                    }

                    G_RecordLocalPerPlayerPickup(vm.playerNum, vm.spriteNum);
                    branch(true);
                    dispatch();
                }

                if (G_ClassicWeaponStayEnabled() && (g_netServer || ud.multimode > 1))
                {
                    if (*insptr == 0)
                    {
                        int j = 0;
...
{
    int effectivePlayerNum  = playerNum;
    int effectivePlayerDist = playerDist;

    if ((unsigned) spriteNum < MAXSPRITES)
    {
        auto const pSprite = &sprite[spriteNum];

        if (G_LocalPerPlayerPickupsEnabled() && pSprite->owner == spriteNum && G_IsLocalPerPlayerPickup(pSprite->picnum))
            effectivePlayerNum = G_FindPlayerForLocalPickup(pSprite, &effectivePlayerDist);
    }

    // for some reason this is faster than using the C++ syntax; e.g vm = vmstate_t{ ... }
    vmstate_t const tempvm
    = { spriteNum, effectivePlayerNum, effectivePlayerDist, 0, &sprite[spriteNum], &actor[spriteNum].t_data[0],
        g_player[effectivePlayerNum].ps, &actor[spriteNum] };
    vm = tempvm;
    ...
    if ((vm.flags & VM_KILL) != 0 && G_HandleLocalPerPlayerPickup(vm.playerNum, vm.spriteNum))
        vm.flags &= ~VM_KILL;
