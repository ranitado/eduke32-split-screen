...
static int32_t Menu_GetPendingSimpleGameMode(void);
...
static MenuMenuFormat_t MMF_Top_GameModeCentered = { {  MENU_MARGIN_CENTER<<16, 70<<16, }, -(202<<16) };
...
static constexpr int32_t SAVELOAD_LAYOUT_YSHIFT = -8;
static MenuMenuFormat_t MMF_LoadSave =             { {                 200<<16, (49+SAVELOAD_LAYOUT_YSHIFT)<<16, },    180<<16 };
...
static MenuEntryFormat_t MEF_PlayerNarrow =     { 1<<16,      0,    132<<16 };
...
MAKE_MENU_TOP_ENTRYLINK( s_NewGame, MEF_MainMenu, MAIN_NEWGAME, MENU_GAMEMODE );
#ifdef EDUKE32_RETAIL_MENU
MAKE_MENU_TOP_ENTRYLINK( "Resume Game", MEF_MainMenu, MAIN_RESUMEGAME, MENU_CLOSE );
#endif
MAKE_MENU_TOP_ENTRYLINK( s_NewGame, MEF_MainMenu, MAIN_NEWGAME_INGAME, MENU_GAMEMODE );
...
#endif
MAKE_MENU_TOP_ENTRYLINK( "Main Menu", MEF_MainMenu, MAIN_QUITTOTITLE, MENU_NULL );
...
static MenuLink_t MEO_GAMEMODE_SINGLE = { MENU_SKILL, MA_Advance, };
static MenuLink_t MEO_GAMEMODE_COOPERATIVE = { MENU_PLAYERCOUNT, MA_Advance, };
static MenuLink_t MEO_GAMEMODE_DUKEMATCH = { MENU_PLAYERCOUNT, MA_Advance, };
static MenuEntry_t ME_GAMEMODE_SINGLE = MAKE_MENUENTRY( "Single Player", &MF_Redfont, &MEF_CenterMenu, &MEO_GAMEMODE_SINGLE, Link );
static MenuEntry_t ME_GAMEMODE_COOPERATIVE = MAKE_MENUENTRY( "Cooperative", &MF_Redfont, &MEF_CenterMenu, &MEO_GAMEMODE_COOPERATIVE, Link );
static MenuEntry_t ME_GAMEMODE_DUKEMATCH = MAKE_MENUENTRY( "DukeMatch", &MF_Redfont, &MEF_CenterMenu, &MEO_GAMEMODE_DUKEMATCH, Link );
static MenuEntry_t *MEL_GAMEMODE[] = {
    &ME_GAMEMODE_SINGLE,
    &ME_GAMEMODE_COOPERATIVE,
    &ME_GAMEMODE_DUKEMATCH,
};

static MenuLink_t MEO_PLAYERCOUNT_2 = { MENU_SKILL, MA_Advance, };
static MenuLink_t MEO_PLAYERCOUNT_3 = { MENU_SKILL, MA_Advance, };
static MenuLink_t MEO_PLAYERCOUNT_4 = { MENU_SKILL, MA_Advance, };
static MenuEntry_t ME_PLAYERCOUNT_2 = MAKE_MENUENTRY( "2 Players", &MF_Redfont, &MEF_CenterMenu, &MEO_PLAYERCOUNT_2, Link );
static MenuEntry_t ME_PLAYERCOUNT_3 = MAKE_MENUENTRY( "3 Players", &MF_Redfont, &MEF_CenterMenu, &MEO_PLAYERCOUNT_3, Link );
static MenuEntry_t ME_PLAYERCOUNT_4 = MAKE_MENUENTRY( "4 Players", &MF_Redfont, &MEF_CenterMenu, &MEO_PLAYERCOUNT_4, Link );
static MenuEntry_t *MEL_PLAYERCOUNT[] = {
    &ME_PLAYERCOUNT_2,
    &ME_PLAYERCOUNT_3,
    &ME_PLAYERCOUNT_4,
};
...
static MenuOption_t MEO_CONTROLS_AIM_AUTO = MAKE_MENUOPTION( &MF_Redfont, &MEOS_GAMESETUP_AIM_AUTO, NULL );
static MenuEntry_t ME_CONTROLS_AIM_AUTO = MAKE_MENUENTRY( "Auto aim:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_CONTROLS_AIM_AUTO, Option );
...
static MenuOptionSet_t MEOS_CONTROLS_PLAYER = MAKE_MENUOPTIONSET( MEOSN_CONTROLS_PLAYER, MEOSV_CONTROLS_PLAYER, 0x0 );
static MenuOption_t MEO_CONTROLS_PLAYER = MAKE_MENUOPTION( &MF_Redfont, &MEOS_CONTROLS_PLAYER, &g_menuPlayerSetupPlayer );
static MenuEntry_t ME_CONTROLS_PLAYER = MAKE_MENUENTRY( "Edit player:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_CONTROLS_PLAYER, Option );

static MenuOption_t MEO_GAMESETUP_FRIENDLYFIRE = MAKE_MENUOPTION( &MF_Redfont, &MEOS_OffOn, &ud.m_ffire );
static MenuEntry_t ME_GAMESETUP_FRIENDLYFIRE = MAKE_MENUENTRY( "Friendly fire:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_GAMESETUP_FRIENDLYFIRE, Option );

static MenuOption_t MEO_GAMESETUP_WEAPONSHARING = MAKE_MENUOPTION( &MF_Redfont, &MEOS_OffOn, &ud.m_weaponsharing );
static MenuEntry_t ME_GAMESETUP_WEAPONSHARING = MAKE_MENUENTRY( "Per-player pickups:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_GAMESETUP_WEAPONSHARING, Option );
...
static MenuOption_t MEO_CONTROLS_EQUIP_PICKUPS = MAKE_MENUOPTION( &MF_Redfont, &MEOS_GAMESETUP_WEAPSWITCH_PICKUP, NULL );
static MenuEntry_t ME_CONTROLS_EQUIP_PICKUPS = MAKE_MENUENTRY( "Equip pickups:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_CONTROLS_EQUIP_PICKUPS, Option );
...
static char const *MEOSN_JOYSTICK_CONTROLLER[] = { "Player 1", "Player 2", "Player 3", "Player 4", };
static int32_t MEOSV_JOYSTICK_CONTROLLER[] = { 0, 1, 2, 3, };
static MenuOptionSet_t MEOS_JOYSTICK_CONTROLLER = MAKE_MENUOPTIONSET(MEOSN_JOYSTICK_CONTROLLER, MEOSV_JOYSTICK_CONTROLLER, 0x0);
static MenuOption_t MEO_JOYSTICK_CONTROLLER = MAKE_MENUOPTION(&MF_Redfont, &MEOS_JOYSTICK_CONTROLLER, &g_menuJoystickController);
static MenuEntry_t ME_JOYSTICK_CONTROLLER = MAKE_MENUENTRY("Edit player:", &MF_Redfont, &MEF_BigOptionsRtSections, &MEO_JOYSTICK_CONTROLLER, Option);
...
static MenuRangeFloat_t MEO_JOYSTICKAXIS_SENSITIVITY = MAKE_MENURANGE( NULL, &MF_Redfont, .2f, 10.f, 10.f, 50, DisplayTypeInteger|EnforceIntervals );
...
#define MIN_SPLIT_BRIGHTNESS 1.f
#define MAX_SPLIT_BRIGHTNESS 4.f
#ifndef EDUKE32_RETAIL_MENU
...
static MenuRangeFloat_t MEO_COLCORR_SPLITBRIGHT = MAKE_MENURANGE( &r_splitscreen_brightness, &MF_Bluefont, MIN_SPLIT_BRIGHTNESS, MAX_SPLIT_BRIGHTNESS, 0.f, 61, DisplayTypePercent );
static MenuEntry_t ME_COLCORR_SPLITBRIGHT = MAKE_MENUENTRY( "Luminosity:", &MF_Redfont, &MEF_BigOptionsRt, &MEO_COLCORR_SPLITBRIGHT, RangeFloat );
#endif
...
static char const *MEOSN_PLAYER_SETUP_PLAYER[] = { "Player 1", "Player 2", "Player 3", "Player 4", };
static int32_t MEOSV_PLAYER_SETUP_PLAYER[] = { 0, 1, 2, 3, };
static MenuOptionSet_t MEOS_PLAYER_SETUP_PLAYER = MAKE_MENUOPTIONSET( MEOSN_PLAYER_SETUP_PLAYER, MEOSV_PLAYER_SETUP_PLAYER, 0x0 );
static MenuOption_t MEO_PLAYER_SETUP_PLAYER = MAKE_MENUOPTION( &MF_Bluefont, &MEOS_PLAYER_SETUP_PLAYER, &g_menuPlayerSetupPlayer );
static MenuEntry_t ME_PLAYER_SETUP_PLAYER = MAKE_MENUENTRY( "Edit player:", &MF_Bluefont, &MEF_PlayerNarrow, &MEO_PLAYER_SETUP_PLAYER, Option );
...
static Menu_t Menus[] = {
    ...
    { &M_JOYSTICKBTNS, MENU_JOYSTICKBTNS, MENU_JOYSTICKADV, MA_Return, Menu },
...
static int Menu_GetSelectedJoystickController(void)
{
    g_menuJoystickController = clamp<int32_t>(g_menuJoystickController, 0, MAXSPLITSCREENCONTROLLERS - 1);
    return g_menuJoystickController;
}

static int32_t (*Menu_GetSelectedJoystickFunctions(void))[2]
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? ud.config.JoystickFunctions : ud.config.SplitScreenJoystickFunctions[controllerIndex - 1];
}

static int32_t (*Menu_GetSelectedJoystickDigitalFunctions(void))[2]
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? ud.config.JoystickDigitalFunctions : ud.config.SplitScreenJoystickDigitalFunctions[controllerIndex - 1];
}

static int32_t *Menu_GetSelectedJoystickAnalogueAxes(void)
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? ud.config.JoystickAnalogueAxes : ud.config.SplitScreenJoystickAnalogueAxes[controllerIndex - 1];
}

static float *Menu_GetSelectedJoystickAnalogueSensitivity(void)
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? ud.config.JoystickAnalogueSensitivity : ud.config.SplitScreenJoystickAnalogueSensitivity[controllerIndex - 1];
}

static int32_t *Menu_GetSelectedJoystickAnalogueInvert(void)
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? ud.config.JoystickAnalogueInvert : ud.config.SplitScreenJoystickAnalogueInvert[controllerIndex - 1];
}

static int32_t *Menu_GetSelectedJoystickAnalogueDead(void)
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? ud.config.JoystickAnalogueDead : ud.config.SplitScreenJoystickAnalogueDead[controllerIndex - 1];
}

static int32_t *Menu_GetSelectedJoystickAnalogueSaturate(void)
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? ud.config.JoystickAnalogueSaturate : ud.config.SplitScreenJoystickAnalogueSaturate[controllerIndex - 1];
}

static int32_t *Menu_GetSelectedJoystickAimWeight(void)
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? &ud.config.JoystickAimWeight : &ud.config.SplitScreenJoystickAimWeight[controllerIndex - 1];
}

static int32_t *Menu_GetSelectedJoystickViewCentering(void)
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
    return controllerIndex == 0 ? &ud.config.JoystickViewCentering : &ud.config.SplitScreenJoystickViewCentering[controllerIndex - 1];
}

static int32_t *Menu_GetSelectedJoystickAimAssist(void)
{
    int const controllerIndex = Menu_GetSelectedJoystickController();
...
static void Menu_UpdateJoystickAimEntries(void)
{
    ...
    auto analogAxes        = Menu_GetSelectedJoystickAnalogueAxes();
    auto analogSensitivity = Menu_GetSelectedJoystickAnalogueSensitivity();
    auto analogInvert      = Menu_GetSelectedJoystickAnalogueInvert();

    for (int i=0;i<MAXJOYAXES;i++)
    {
        if (analogAxes[i] == analog_turning)
        {
            MEO_JOYSTICK_HORIZONTALAIMSENSITIVITY.variable = &analogSensitivity[i];
            ME_JOYSTICK_HORIZONTALAIMSENSITIVITY.flags &= ~MEF_Hidden;
            g_turnAxis = i;
            break;
        }
    ...
    for (int i=0;i<MAXJOYAXES;i++)
    {
        if (analogAxes[i] == analog_lookingupanddown)
        {
            MEO_JOYSTICK_VERTICALAIMSENSITIVITY.variable = &analogSensitivity[i];
            ME_JOYSTICK_VERTICALAIMSENSITIVITY.flags &= ~MEF_Hidden;
            MEO_JOYSTICK_LOOKINVERT.data = &analogInvert[i];
            ME_JOYSTICK_LOOKINVERT.flags &= ~MEF_Hidden;
            g_lookAxis = i;
            break;
        }
...
{
    auto joystickFunctions = Menu_GetSelectedJoystickFunctions();
    auto joystickDigitalFunctions = Menu_GetSelectedJoystickDigitalFunctions();
    ...
    for (int i = 0; i < joystick.numButtons + 4*joystick.numHats + 2*joystick.isGameController; ++i)
    {
        ...
        if (i < joystick.numButtons + 4*joystick.numHats)
            MEO_JOYSTICKBTNS[i].data = &joystickFunctions[i][0];
        else if (joystick.isGameController)
            MEO_JOYSTICKBTNS[i].data = &joystickDigitalFunctions[i - joystick.numButtons - 4*joystick.numHats + 4][1];
    }
...
static int Menu_GetSelectedPlayerSetupPlayer(void)
{
    g_menuPlayerSetupPlayer = clamp<int32_t>(g_menuPlayerSetupPlayer, 0, MAXSPLITSCREENCONTROLLERS - 1);
    return g_menuPlayerSetupPlayer;
}

static int Menu_IsEditingPrimaryPlayerSetup(void)
{
    return Menu_GetSelectedPlayerSetupPlayer() == 0;
}

static int32_t *Menu_GetSelectedPlayerColor(void)
{
    int const playerNum = Menu_GetSelectedPlayerSetupPlayer();
    return playerNum == 0 ? &ud.color : &ud.config.SplitScreenPlayerColor[G_GetSplitScreenPlayerConfigIndex(playerNum)];
}

static int32_t *Menu_GetSelectedPlayerTeam(void)
{
    int const playerNum = Menu_GetSelectedPlayerSetupPlayer();
    return playerNum == 0 ? &ud.team : &ud.config.SplitScreenPlayerTeam[G_GetSplitScreenPlayerConfigIndex(playerNum)];
}

static char *Menu_GetSelectedPlayerName(void)
{
    int const playerNum = Menu_GetSelectedPlayerSetupPlayer();
    return playerNum == 0 ? szPlayerName : ud.config.SplitScreenPlayerName[G_GetSplitScreenPlayerConfigIndex(playerNum)];
}

static int32_t *Menu_GetSelectedPlayerAutoAim(void)
{
    int const playerNum = Menu_GetSelectedPlayerSetupPlayer();
...
{
    int const playerNum = Menu_GetSelectedPlayerSetupPlayer();
    return playerNum == 0 ? &ud.weaponswitch : &ud.config.SplitScreenPlayerWeaponSwitch[G_GetSplitScreenPlayerConfigIndex(playerNum)];
}

static int32_t Menu_GetEquipPickupOption(int32_t const weaponSwitch)
{
    return (weaponSwitch & 1) ? ((weaponSwitch & 4) ? 2 : 1) : 0;
}

static void Menu_SetSelectedPlayerEquipPickupOption(int32_t const newOption)
{
    int32_t * const weaponSwitch = Menu_GetSelectedPlayerWeaponSwitch();
    *weaponSwitch &= ~(1|4);
...
static void Menu_ApplySelectedPlayerSetup(void)
{
    ...
    if (g_player[playerNum].ps != nullptr)
    {
        ...
        int const team = *Menu_GetSelectedPlayerTeam();
        int const color = *Menu_GetSelectedPlayerColor();
        int const palette = (g_gametypeFlags[ud.coop] & GAMETYPE_TDM) ? G_GetTeamPalette(team) : color;

        Bstrncpyz(g_player[playerNum].user_name, Menu_GetSelectedPlayerName(), sizeof(g_player[playerNum].user_name));
...
{
    MEO_CONTROLS_AIM_AUTO.data = Menu_GetSelectedPlayerAutoAim();
}
...
void Menu_Init(void)
{
    ...
    M_GAMEMODE.currentEntry = clamp(Menu_GetSelectedSimpleGameMode(), 0, M_GAMEMODE.numEntries - 1);
...
static void Menu_Pre(MenuID_t cm)
{
...

    {
        ...
        if (realMultiplayer && ud.recstat != 2)
        {
...
//# endif
    ...
    case MENU_CONTROLS:
        Menu_UpdateControlSetupEntries();
        break;
...
static void msaveloadtext(const vec2_t& origin, int level, int volume, int skill, const char *boardfn, int16_t /*health*/, int32_t playerCount)
{
    ...
    Menu_BlackRectangle(origin.x + ((xoffset-2)<<16), origin.y + ((yoffset-2)<<16), 178<<16, boxHeight<<16, 1);
...
static void Menu_DrawSplitScreenHelp(const vec2_t origin)
{
    ...
    creditsminitext(origin.x + (84<<16), origin.y + (48<<16), "CONTROLLER", 12);
    creditsminitext(origin.x + (236<<16), origin.y + (48<<16), "KEYBOARD / MOUSE", 12);

    static char const * controller[] =
    {
        "LS        MOVE / STRAFE",
        "RS        LOOK / AIM",
        "LT        SLOW AIM / RUN",
        ...
        "DPAD      INVENTORY / ITEMS",
        "BACK      MAP",
        "START     PAUSE MENU",
        "RS CLICK  CENTER VIEW",
    };
    ...
    for (int i = 0; i < ARRAY_SSIZE(controller); ++i)
    {
        int const y = 60 + i * 7;
        mminitext(origin.x + (22<<16), origin.y + (y<<16), controller[i], 8);
...
static void Menu_PreDraw(MenuID_t cm, MenuEntry_t* entry, const vec2_t origin)
{
    ...
    case MENU_MAIN:
        if ((G_GetLogoFlags() & LOGO_NOGAMETITLE) == 0)
        {
            ...
            {
                int32_t const logoTile = PLUTOPAKSPRITE + 2;
                int32_t const logoZoom = tilesiz[logoTile].x > 0 ? divscale16(95, tilesiz[logoTile].x) : 65536L;
                rotatesprite_fs(origin.x + ((MENU_MARGIN_CENTER+100)<<16), origin.y + (36<<16), logoZoom,0,logoTile,(sintable[(timer120()<<4)&2047]>>11),0,2+8);
            }
        }
        break;
    ...
    case MENU_PLAYER:
    {
        int const color = *Menu_GetSelectedPlayerColor();
        int const team = *Menu_GetSelectedPlayerTeam();
        rotatesprite_fs(origin.x + (260<<16), origin.y + ((24+(tilesiz[APLAYER].y>>1))<<16), 49152L,0,1441-((((4-(timer120()>>4)))&3)*5),0,entry == &ME_PLAYER_TEAM ? G_GetTeamPalette(team) : color,10);
        break;
    }
    ...
    case MENU_LOAD:
    {
        ...
        rotatesprite_fs(origin.x + (99<<16), origin.y + ((50+SAVELOAD_LAYOUT_YSHIFT)<<16), 65536L,512,WINDOWBORDER1,24,0,10);
        rotatesprite_fs(origin.x + (103<<16), origin.y + ((144+SAVELOAD_LAYOUT_YSHIFT)<<16), 65536L,1024+512,WINDOWBORDER1,24,0,10);

        if (M_LOAD.currentEntry >= (int32_t)g_nummenusaves)
        {
            menutext_centeralign(origin.x + (101<<16), origin.y + ((97+SAVELOAD_LAYOUT_YSHIFT)<<16), "Empty");
            break;
        }
        ...
        if (msv.brief.isValid())
        {
            if (waloff[TILE_LOADSHOT])
                rotatesprite_fs(origin.x + (101<<16), origin.y + ((97+SAVELOAD_LAYOUT_YSHIFT)<<16),
                                65536>>1,512,TILE_LOADSHOT, msv.isOldVer?16:-32, 0,4+10+64);

            if (msv.isOldVer)
            {
                mgametextcenterat(origin.x + (101<<16), origin.y + ((64+SAVELOAD_LAYOUT_YSHIFT)<<16),
                ...
                mminitext(origin.x + (23<<16), origin.y + ((124+SAVELOAD_LAYOUT_YSHIFT)<<16), tempbuf, MF_Minifont.pal_selected);
                ...
                mminitext(origin.x + (23<<16), origin.y + ((132+SAVELOAD_LAYOUT_YSHIFT)<<16), tempbuf, MF_Minifont.pal_selected);
#endif
    ...
    case MENU_SAVE:
    {
        ...
        if (j)
            rotatesprite_fs(origin.x + (101<<16), origin.y + ((97+SAVELOAD_LAYOUT_YSHIFT)<<16),
                            65536L>>1,512,TILE_SAVESHOT,-32,0,4+10+64);
        else if (0 < M_SAVE.currentEntry && M_SAVE.currentEntry <= (int32_t)g_nummenusaves)
        {
            ...
            if (msv.brief.isValid())
            {
                ...
                if (msv.isOldVer)
                {
                    mgametextcenterat(origin.x + (101<<16), origin.y + ((50+SAVELOAD_LAYOUT_YSHIFT)<<16),
                    ...
                    mgametext(origin.x + (25<<16), origin.y + ((124+SAVELOAD_LAYOUT_YSHIFT)<<16), tempbuf);
                    Bsprintf(tempbuf,"Our: %d.%d.%d.%u %d-bit", SV_MAJOR_VER, SV_MINOR_VER, BYTEVERSION,
                             ud.userbytever, (int32_t)(8*sizeof(intptr_t)));
                    mgametext(origin.x + ((25+16)<<16), origin.y + ((134+SAVELOAD_LAYOUT_YSHIFT)<<16), tempbuf);
#endif
                }
            }
        }
    ...
    {
#define MENU_YOFFSET 34
#define MENU_INCREMENT(x) (oy += ((x) << 16))  // maybe this should have been MENU_EXCREMENT instead

        int32_t oy = origin.y;

        mgametextcenter(origin.x, MENU_INCREMENT(MENU_YOFFSET), "SPLIT-SCREEN Mod");
        creditsminitext(origin.x + (160 << 16), MENU_INCREMENT(11), "Ranitado - Dev", 8);

        mgametextcenter(origin.x, MENU_INCREMENT(11), "EDuke32 Developers");
...
enum MenuSimpleGameMode_t
{
    MENU_SIMPLE_GAMEMODE_SINGLE = 0,
    MENU_SIMPLE_GAMEMODE_COOPERATIVE,
    MENU_SIMPLE_GAMEMODE_DUKEMATCH,
};
...
static int32_t Menu_ClampLocalPlayerCount(int32_t const playerCount)
{
    return clamp(playerCount, 2, 4);
}

static int32_t Menu_GetCurrentLocalPlayerCount(void)
{
    return max<int32_t>(G_GetSplitScreenPlayerCount(), 1);
}

static int32_t Menu_GetDefaultLocalPlayerCount(void)
{
    return Menu_ClampLocalPlayerCount(G_GetConfiguredSplitScreenPlayerCount());
}

static int32_t Menu_GetSelectedSimpleGameMode(void)
{
    if (Menu_GetCurrentLocalPlayerCount() <= 1)
        return MENU_SIMPLE_GAMEMODE_SINGLE;

    return ud.m_coop == 1 ? MENU_SIMPLE_GAMEMODE_COOPERATIVE : MENU_SIMPLE_GAMEMODE_DUKEMATCH;
}
...
static void Menu_ResetPendingSimpleGameState(int32_t const forceDefaultMode)
{
    ...
    g_simplePendingLocalPlayerCount = Menu_GetCurrentLocalPlayerCount() > 1
        ? Menu_GetCurrentLocalPlayerCount()
        : Menu_GetDefaultLocalPlayerCount();
}
...
{
    MMF_Top_Episode.pos.y = (58 + (4 - clamp(entryCount, 0, MAXLEVELS)) * 6) << 16;
}

static void Menu_PopulateSimpleLevelMenu(int32_t const volumeIndex)
{
    int levelCount = 0;
    ...
    if ((unsigned) volumeIndex < MAXVOLUMES)
    {
        for (int levelIndex = 0; levelIndex < MAXLEVELS; ++levelIndex)
        {
            ...
            MEL_LEVEL[levelCount] = &ME_LEVEL[levelCount];
            ME_LEVEL[levelCount] = ME_LEVEL_TEMPLATE;
            ME_LEVEL[levelCount].name = mapInfo.name;
            g_simpleLevelIndices[levelCount] = levelIndex;
            ++levelCount;
        }
    }
    ...
    if (levelCount <= 0)
    {
        MEL_LEVEL[0] = &ME_LEVEL[0];
        ME_LEVEL[0] = ME_LEVEL_TEMPLATE;
        ME_LEVEL[0].name = s_Undefined;
        g_simpleLevelIndices[0] = 0;
        levelCount = 1;
    }

    M_LEVELSELECT.numEntries = levelCount;
    M_LEVELSELECT.currentEntry = 0;

    for (int entryIndex = 0; entryIndex < levelCount; ++entryIndex)
    {
            if (g_simpleLevelIndices[entryIndex] == g_simplePendingLevel)
            {
                M_LEVELSELECT.currentEntry = entryIndex;
                break;
            }
    }
...
static int32_t Menu_GetSelectedSimpleLevel(void)
{
    if (M_LEVELSELECT.currentEntry < 0 || M_LEVELSELECT.currentEntry >= M_LEVELSELECT.numEntries)
        return 0;

    return g_simpleLevelIndices[M_LEVELSELECT.currentEntry];
}

static void Menu_ApplySimpleGameMode(int32_t const mode)
{
    int const localPlayerCount = mode == MENU_SIMPLE_GAMEMODE_SINGLE ? 1 : Menu_GetPendingLocalPlayerCount();

    if (mode == MENU_SIMPLE_GAMEMODE_SINGLE)
        G_SetActiveSplitScreenPlayerCount(1);
    else
    {
        G_SetConfiguredSplitScreenPlayerCount(localPlayerCount);
        G_SetActiveSplitScreenPlayerCount(localPlayerCount);
    }

    g_mostConcurrentPlayers = localPlayerCount;
    ud.multimode = localPlayerCount;
    ...
    switch (mode)
    {
    case MENU_SIMPLE_GAMEMODE_COOPERATIVE:
        ud.m_coop = 1;
        break;

    case MENU_SIMPLE_GAMEMODE_DUKEMATCH:
        ud.m_coop = 0;
        ud.m_marker = 1;
        ud.m_ffire = 1;
...
static void Menu_EntryLinkActivate(MenuEntry_t *entry)
{
    if (entry->type == Link)
    {
        auto const * const link = (MenuLink_t const *)entry->entry;
        if (link->linkID == MENU_SAVE && g_player[myconnectindex].ps != nullptr
    ...
    {
        g_forceSimpleGameModeDefault = true;
        Menu_ResetPendingSimpleGameState(true);
    }
    ...
    case MENU_JOYSTICKAXES:
    {
        auto analogAxes        = Menu_GetSelectedJoystickAnalogueAxes();
        auto analogInvert      = Menu_GetSelectedJoystickAnalogueInvert();
        auto analogDead        = Menu_GetSelectedJoystickAnalogueDead();
        auto analogSaturate    = Menu_GetSelectedJoystickAnalogueSaturate();
        auto analogSensitivity = Menu_GetSelectedJoystickAnalogueSensitivity();
        auto digitalFunctions  = Menu_GetSelectedJoystickDigitalFunctions();

        M_JOYSTICKAXIS.title = joyGetName(0, M_JOYSTICKAXES.currentEntry);
        MEO_JOYSTICKAXIS_ANALOG.data = &analogAxes[M_JOYSTICKAXES.currentEntry];
        MEO_JOYSTICKAXIS_INVERT.data = &analogInvert[M_JOYSTICKAXES.currentEntry];
        MEO_JOYSTICKAXIS_DEAD.variable = &analogDead[M_JOYSTICKAXES.currentEntry];
        MEO_JOYSTICKAXIS_SATU.variable = &analogSaturate[M_JOYSTICKAXES.currentEntry];
        MEO_JOYSTICKAXIS_SENSITIVITY.variable = &analogSensitivity[M_JOYSTICKAXES.currentEntry];
        MEO_JOYSTICKAXIS_DIGITALNEGATIVE.data = &digitalFunctions[M_JOYSTICKAXES.currentEntry][0];
        MEO_JOYSTICKAXIS_DIGITALPOSITIVE.data = &digitalFunctions[M_JOYSTICKAXES.currentEntry][1];
    ...
    else if (entry == &ME_NETHOST_LAUNCH)
    {
        ...
        else if (G_HaveSplitScreen() && !g_netClient)
        {
            Net_FillNewGame(&pendingnewgame, 1);
            Net_StartNewGame();
        }
...
static int32_t Menu_EntryOptionModify(MenuEntry_t *entry, int32_t newOption)
{
    ...
    else if (entry == &ME_PLAYER_EQUIP_PICKUPS || entry == &ME_CONTROLS_EQUIP_PICKUPS)
        Menu_SetSelectedPlayerEquipPickupOption(newOption);
    else if (entry == &ME_GAMESETUP_FRIENDLYFIRE)
    {
        ud.ffire = newOption;
    }
    else if (entry == &ME_GAMESETUP_WEAPONSHARING)
    {
        ud.weaponsharing = newOption;
    }
    ...
    {
        g_menuJoystickController = newOption;
        Menu_UpdateJoystickAimEntries();
        Menu_UpdateJoystickAdvancedEntries();
        Menu_PopulateJoystick();
    }
    else if (entry == &ME_PLAYER_SETUP_PLAYER || entry == &ME_CONTROLS_PLAYER)
    {
        g_menuPlayerSetupPlayer = newOption;
        Menu_UpdatePlayerSetupEntries();
        Menu_UpdateControlSetupEntries();
    }
    else if (entry == &ME_JOYSTICKAXIS_ANALOG)
    {
        if (Menu_IsEditingPrimaryJoystick())
...
static int32_t Menu_EntryRangeFloatModify(MenuEntry_t *entry, float newValue)
{
    ...
    else if (entry == &ME_COLCORR_SPLITBRIGHT)
        r_splitscreen_brightnessrecip = 1.f/newValue;
    else
#else
...
static void Menu_BuildAutoSaveName(char * const name, size_t const nameSize)
{
    ...
    char suffix[8];
    Bsnprintf(suffix, sizeof(suffix), " %dP", Menu_GetSavePlayerCount());

    char const * const levelName = g_mapInfo[ud.volume_number * MAXLEVELS + ud.level_number].name;
    int32_t const levelNameMaxLen = max<int32_t>(0, MAXSAVEGAMENAME - Bstrlen(suffix));
    Bsnprintf(name, nameSize, "%.*s%s", levelNameMaxLen, levelName ? levelName : "Unknown", suffix);
}
...
static int32_t Menu_SaveCurrentEntry(char const *input)
{
    ...
    bool const overwriteExistingSave = M_SAVE.currentEntry > 0;
    savebrief_t & sv = g_lastusersave = M_SAVE.currentEntry == 0 ? savebrief_t{ saveName } : g_menusaves[M_SAVE.currentEntry - 1].brief;
...
static int32_t Menu_EntryOptionSource(MenuEntry_t *entry, int32_t currentValue)
{
    if (entry == &ME_GAMESETUP_WEAPSWITCH_PICKUP)
        return Menu_GetEquipPickupOption(ud.weaponswitch);
    else if (entry == &ME_PLAYER_EQUIP_PICKUPS || entry == &ME_CONTROLS_EQUIP_PICKUPS)
        return Menu_GetEquipPickupOption(*Menu_GetSelectedPlayerWeaponSwitch());
    else if (entry == &ME_SOUND_DUKETALK)
...
static void Menu_Verify(int32_t input)
{
    ...
    case MENU_SAVEVERIFY:
        if (input)
        {
            ...
            Menu_StopTextInput();
            Menu_SaveCurrentEntry(saveName);
    ...
    case MENU_COLCORRRESETVERIFY:
        if (input)
        {
            ...
            r_splitscreen_brightness = 3.f;
            r_splitscreen_brightnessrecip = 1.f / r_splitscreen_brightness;
            videoSetPalette(ud.brightness>>2,g_player[myconnectindex].ps->palette,0);
        }
        break;
    ...
    case MENU_JOYDEFAULTVERIFY:
        if (input)
        {
            if (Menu_IsEditingPrimaryJoystick())
                ...
                CONFIG_SetSplitScreenGameControllerDefaults(Menu_GetSelectedJoystickController());
...
static void Menu_ChangingTo(Menu_t * m)
{
    ...
    {
    case MENU_GAMEMODE:
        if (!Menu_HaveSimpleNewGameFlow(g_previousMenu) || g_forceSimpleGameModeDefault)
        {
            Menu_ResetPendingSimpleGameState(g_forceSimpleGameModeDefault);
            g_forceSimpleGameModeDefault = false;
        }
...
static void Menu_RunInput_EntryString_Submit(/*MenuEntry_t *entry, */MenuString_t *object)
{
    ...
    if (g_currentMenu == MENU_PLAYER && object == &MEO_PLAYER_NAME)
    {
        CommandName = nullptr;
        Menu_ApplySelectedPlayerSetup();
    }
        ...
        case List:
        {
            ...
            if (state == 0)
            {
                ...
                if (!entryActivated && (I_ReturnTrigger() || I_EscapeTrigger() || Menu_RunInput_MouseReturn()))
                {
                    ...
                    if (cm->menuID == MENU_MAIN && (g_player[myconnectindex].ps->gm & MODE_GAME) == 0)
                    {
                        g_noLogoAnim = 0;
                        G_DisplayLogo();
                    }
