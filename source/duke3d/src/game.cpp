...
{
    return ud.weaponsharing && ud.coop == 1 && g_fakeMultiMode > 1;
}

int32_t G_IsLocalPerPlayerPickup(int32_t picnum)
{
...
int32_t G_PlayerHasLocalPerPlayerPickup(int32_t playerNum, int32_t spriteNum)
{
    if (!G_LocalPerPlayerPickupsEnabled())
        return 0;

    if ((unsigned) playerNum >= MAXPLAYERS || (unsigned) spriteNum >= MAXSPRITES)
        return 0;

    return bitmap_test(g_localPerPlayerPickupMask[playerNum], spriteNum);
}

void G_RecordLocalPerPlayerPickup(int32_t playerNum, int32_t spriteNum)
{
    if ((unsigned) playerNum >= MAXPLAYERS || (unsigned) spriteNum >= MAXSPRITES)
        return;

    bitmap_set(g_localPerPlayerPickupMask[playerNum], spriteNum);
}

void G_ResetLocalPerPlayerPickups(void)
{
    Bmemset(g_localPerPlayerPickupMask, 0, sizeof(g_localPerPlayerPickupMask));
}

int32_t G_AreLocalPerPlayerPickupsComplete(int32_t spriteNum)
{
    if (!G_LocalPerPlayerPickupsEnabled())
        return 1;
    ...
    for (int viewIndex = 0; viewIndex < playerCount; ++viewIndex)
    {
        ...
        if (!G_PlayerHasLocalPerPlayerPickup(playerNum, spriteNum))
            return 0;
    }
...
int32_t G_HandleLocalPerPlayerPickup(int32_t playerNum, int32_t spriteNum)
{
    if (!G_LocalPerPlayerPickupsEnabled())
        return 0;
    ...
    if (pSprite->owner != spriteNum || !G_IsLocalPerPlayerPickup(pSprite->picnum))
        return 0;

    G_RecordLocalPerPlayerPickup(playerNum, spriteNum);
    return !G_AreLocalPerPlayerPickupsComplete(spriteNum);
}

int32_t G_FindPlayerForLocalPickup(spritetype const *pSprite, int32_t *dist)
{
    if (pSprite == nullptr || !G_LocalPerPlayerPickupsEnabled())
        return A_FindPlayer(pSprite, dist);

    int const spriteNum = pSprite - sprite;
    if ((unsigned) spriteNum >= MAXSPRITES || pSprite->owner != spriteNum || !G_IsLocalPerPlayerPickup(pSprite->picnum))
        return A_FindPlayer(pSprite, dist);

    int     closestPlayer     = -1;
    int32_t closestPlayerDist = INT32_MAX;

    for (int viewIndex = 0; viewIndex < G_GetSplitScreenPlayerCount(); ++viewIndex)
    {
        int const playerNum = G_GetSplitScreenPlayer(viewIndex);
        ...
        if (pPlayer == nullptr || G_PlayerHasLocalPerPlayerPickup(playerNum, spriteNum))
            continue;

        int32_t playerDist = klabs(pPlayer->opos.x - pSprite->x)
                             + klabs(pPlayer->opos.y - pSprite->y)
                             + ((klabs(pPlayer->opos.z - pSprite->z + (28 << 8))) >> 4);

        if (sprite[pPlayer->i].extra <= 0)
            playerDist += (INT32_MAX >> 2);
...
static void G_ReadGLFrame(void)
{
    ...
    for (int y = 0; y < 200; y++)
    {
        ...
        const int32_t base = sourceY * xdim;
...
static int32_t g_splitScreenFovOffset;

static int32_t G_GetSplitScreenFovOffset(int const playerCount, int const viewIndex)
{
    if (playerCount == 2 || (playerCount == 3 && viewIndex == 0))
        return 20;
...
void G_DrawRooms(int32_t playerNum, int32_t smoothRatio)
{
    ...
    float visibilityScale = (numplayers > 1) ? 1.f : r_ambientlightrecip;
    visibilityScale *= r_splitscreen_brightnessrecip;

    g_visibility = (playerVis <= 0) ? 0 : (int32_t)(playerVis * visibilityScale);
    ...
    else
    {
        ...
        int32_t const effectiveFov = clamp<int32_t>(ud.fov + g_splitScreenFovOffset, 40, 140);
        vr = Blrintf(float(vr) * tanf(effectiveFov * (fPI/360.f)));
        ...
        if (g_earthquakeTime > 0 && pPlayer->on_ground == 1 && (!G_HaveSplitScreen() || G_GetSplitScreenPlayerCount() <= 1))
        {
...

    {
        ...
        // ud.const_visibility.
        static int32_t lastvist[MAXPLAYERS];
        int const visibilityPlayer = clamp<int>(playerNum, 0, MAXPLAYERS - 1);
        const int32_t visdif = ud.const_visibility-pPlayer->visibility;

        // Check if totalclock was cleared (e.g. restarted game).
        if (totalclock < lastvist[visibilityPlayer])
            lastvist[visibilityPlayer] = 0;

        // Every 2nd totalclock increment (each 1/60th second), ...
        while (totalclock >= lastvist[visibilityPlayer]+2)
        {
            ...
            lastvist[visibilityPlayer] = (int32_t) totalclock;
        }
    }
...
static bool G_DrawSplitScreenRooms(int32_t const smoothRatio)
{
    int const playerCount = G_GetSplitScreenPlayerCount();
    if (playerCount <= 1)
        return false;

    bool primaryCameraSaved = false;
    vec3_t primaryCameraPos {};
    fix16_t primaryCameraAng = 0;
    fix16_t primaryCameraHoriz = 0;
    int16_t primaryCameraSect = -1;
    ...
    for (int viewIndex = 0; viewIndex < playerCount; ++viewIndex)
    {
        ...
        g_splitScreenFovOffset = G_GetSplitScreenFovOffset(playerCount, viewIndex);
        G_DrawRooms(playerNum, smoothRatio);
        g_splitScreenFovOffset = 0;

        if (playerNum == myconnectindex || (!primaryCameraSaved && viewIndex == 0))
        {
            primaryCameraPos = CAMERA(pos);
            primaryCameraAng = CAMERA(q16ang);
            primaryCameraHoriz = CAMERA(q16horiz);
            primaryCameraSect = CAMERA(sect);
            primaryCameraSaved = true;
        }
    }

    if (primaryCameraSaved)
    {
        CAMERA(pos) = primaryCameraPos;
        CAMERA(q16ang) = primaryCameraAng;
        CAMERA(q16horiz) = primaryCameraHoriz;
        CAMERA(sect) = primaryCameraSect;
    }
...
{
    if (g_networkMode == NET_DEDICATED_SERVER || (unsigned)myconnectindex >= MAXPLAYERS || g_player[myconnectindex].ps == nullptr)
        return;
    ...
    bool const splitScreenRendered = G_DrawSplitScreenRooms(smoothRatio);
    ...
    if (!splitScreenRendered && videoGetRenderMode() >= REND_POLYMOST)
        G_DrawBackground();
...
void G_DoSpriteAnimations(int32_t ourx, int32_t oury, int32_t ourz, int32_t oura, int32_t smoothratio)
{
    ...
    for (j=spritesortcnt-1; j>=0; j--) //Between drawrooms() and drawmasks()
    {
        ...
        if (G_LocalPerPlayerPickupsEnabled() && pSprite->owner == i && G_IsLocalPerPlayerPickup(pSprite->picnum)
            && G_PlayerHasLocalPerPlayerPickup(screenpeek, i))
        {
            t->xrepeat = t->yrepeat = 0;
            continue;
        }
...
//    CONTROL_ProcessBinds();
    ...
    if (!G_HaveSplitScreen() && BUTTON(gamefunc_See_Coop_View) && (GTFLAGS(GAMETYPE_COOPVIEW) || ud.recstat == 2))
    {
...
static void G_MergePrimarySplitScreenGamepadInput(input_t const &gamepadInput)
{
    constexpr int32_t maxLocalInputVelocity = ((40 * 2) + 10);  // KEEPINSYNC player.cpp
...
int app_main(int argc, char const* const* argv)
{
    ...
    {
        Bstrncpyz(g_player[j].user_name, ud.config.SplitScreenPlayerName[G_GetSplitScreenPlayerConfigIndex(j)], sizeof(g_player[j].user_name));
        g_player[j].ps->team = g_player[j].pteam = G_GetLocalPlayerTeamSetting(j);
        g_player[j].ps->palookup = g_player[j].pcolor = G_GetLocalPlayerColorSetting(j);
        g_player[j].ps->weaponswitch = G_GetLocalPlayerWeaponSwitchSetting(j);
        g_player[j].ps->auto_aim = G_GetLocalPlayerAutoAimSetting(j);
    }
