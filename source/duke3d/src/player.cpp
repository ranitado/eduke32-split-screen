...
static int32_t G_GetSplitScreenViewportForPlayer(int32_t const playerNum, splitscreen_viewport_t * const viewport)
{
    ...
    int const playerCount = G_GetSplitScreenPlayerCount();
    for (int viewIndex = 0; viewIndex < playerCount; ++viewIndex)
    {
        if (G_GetSplitScreenPlayer(viewIndex) != playerNum)
            continue;

        return G_GetSplitScreenViewport(viewIndex, viewport);
    }

    return -1;
}

static FORCE_INLINE bool G_ShouldPlayPlayerActionSound(int32_t const playerNum)
{
...
static int32_t G_GetSplitScreenControllerProfileForPlayer(int32_t const playerNum)
{
    ...
    for (int viewIndex = 0; viewIndex < playerCount; ++viewIndex)
        if (G_GetSplitScreenPlayer(viewIndex) == playerNum)
            return clamp<int32_t>(viewIndex, 0, MAXSPLITSCREENCONTROLLERS - 1);

    return clamp<int32_t>(playerNum, 0, MAXSPLITSCREENCONTROLLERS - 1);
}

static int32_t G_GetJoystickAimWeightForPlayer(int32_t const playerNum)
{
    int const profile = G_GetSplitScreenControllerProfileForPlayer(playerNum);
    return profile == 0 ? ud.config.JoystickAimWeight : ud.config.SplitScreenJoystickAimWeight[profile - 1];
}
...
static void P_AddForceFeedbackForPlayer(int32_t const playerNum, int const lo, int const hi, int const time)
{
    ...
    if (G_HaveSplitScreen() && playerNum >= 0)
    {
        ...
        joyAddGamepadRumble(gamepadIndex, min<int>(UINT16_MAX, lo), min<int>(UINT16_MAX, hi), max<int>(0, time));
        return;
    }
#else
    ...
    I_AddForceFeedback(lo, hi, time);
}

static int32_t G_GetSplitScreenWeaponYOffsetPixels(int32_t const playerNum)
{
...
static int32_t G_GetSplitScreenScopedHudUniqueId(int32_t const uniqueID)
{
    ...
    int const playerCount = G_GetSplitScreenPlayerCount();
    constexpr int32_t stride = W_END + 1;

    // If we don't have enough unique HUD slots for all local views, disable HUD interpolation
    // rather than reusing the same cache entries across multiple players.
    if (playerCount < 2 || playerCount * stride > MAXUNIQHUDID)
        return 0;

    for (int viewIndex = 0; viewIndex < playerCount; ++viewIndex)
    {
        if (G_GetSplitScreenPlayer(viewIndex) != screenpeek)
            continue;

        return uniqueID + viewIndex * stride;
    }
...
static void P_IncurDamage(DukePlayer_t * const pPlayer)
{
    ...
    P_AddForceFeedbackForPlayer(P_Get(pPlayer->i), (admg << FF_PLAYER_DMG_SCALE), (admg << FF_PLAYER_DMG_SCALE), (admg << FF_PLAYER_TIME_SCALE));
}

void P_QuickKill(DukePlayer_t * const pPlayer)
{
    P_PalFrom(pPlayer, 48, 48,48,48);
    P_AddForceFeedbackForPlayer(P_Get(pPlayer->i), pPlayer->max_player_health << FF_PLAYER_DMG_SCALE,
                                pPlayer->max_player_health << FF_PLAYER_DMG_SCALE, pPlayer->max_player_health << FF_PLAYER_TIME_SCALE);
...
static void P_DoWeaponRumble(int playerNum)
{
    ...
    P_AddForceFeedbackForPlayer(playerNum, (dmg << FF_WEAPON_DMG_SCALE), (dmg << FF_WEAPON_DMG_SCALE), max<int>(FF_WEAPON_MAX_TIME, dmg << FF_WEAPON_TIME_SCALE));
}
...
static void P_DisplaySpit(void)
{
    ...
    for (int i=0; i < loogs; i++)
    {
        ...
        rotatesprite_fs_id((pPlayer->loogie[i].x + rotX) << 16, (200 + pPlayer->loogie[i].y - rotY) << 16, rotZoom - (i << 8),
                        256 - rotAng, LOOGIE, 0, 0, 2, G_GetSplitScreenScopedHudUniqueId(W_LOOGIE + i));
    }
}
#endif
...
static int P_DisplayFist(int const fistShade)
{
    ...
    int const fistY       = 194 + baseY + (sintable[((6 + fistInc) << 7) & 2047] >> 9) + G_GetSplitScreenWeaponYOffsetPixels(screenpeek);
    ...
    guniqhudid = G_GetSplitScreenScopedHudUniqueId(W_FIST);
...
static int32_t G_GetRpgHudTile(void)
{
    if (WORLDTOUR)
        return RPGGUNWIDE;

#ifdef SPLITSCREEN_MOD_HACKS
    static int32_t cachedRpgTile = -1;

    if (cachedRpgTile < 0)
    {
        tileLoad(RPGGUNWIDE);
        cachedRpgTile = (tilesiz[RPGGUNWIDE].x > 0 && tilesiz[RPGGUNWIDE].y > 0) ? RPGGUNWIDE : RPGGUN;
    }

    return cachedRpgTile;
#else
    return RPGGUN;
#endif
}
...
static void G_DrawTileScaled(int drawX, int drawY, int tileNum, int drawShade, int drawBits, int drawPal, int uniqueID = 0)
{
    ...
    guniqhudid = G_GetSplitScreenScopedHudUniqueId(uniqueID);
    drawYOffset += G_GetSplitScreenWeaponYOffsetPixels(screenpeek) << 16;
...
{
    static int shadef[MAXPLAYERS];
    static int palf[MAXPLAYERS];
    int const drawPlayer = clamp<int>(screenpeek, 0, MAXPLAYERS - 1);

    // basic fading between player weapon shades
    if (shadef[drawPlayer] != weaponShade && (!weaponPal || palf[drawPlayer] == weaponPal))
    {
        shadef[drawPlayer] += (weaponShade - shadef[drawPlayer]) >> 2;

        if (!((weaponShade - shadef[drawPlayer]) >> 2))
            shadef[drawPlayer] = logapproach(shadef[drawPlayer], weaponShade);
    }
    else
        shadef[drawPlayer] = weaponShade;

    palf[drawPlayer] = weaponPal;
    ...
    if (videoGetRenderMode() >= REND_POLYMOST)
    {
        if (uniqueID == W_CHAINGUN_TOP)
        {
            if (!usemodels || md_tilehasmodel(weaponTile, weaponPal) < 0)
            {
                ...
                G_DrawTileScaled(weaponX, weaponY, weaponTile, shadef[drawPlayer], weaponBits, weaponPal, W_CHAINGUN_HACK);
                g_dts_yadd = 0;
            }
        }
    }
    ...
    G_DrawTileScaled(weaponX, weaponY, weaponTile, shadef[drawPlayer], weaponBits, weaponPal, uniqueID);
}
#endif
...
void P_DisplayWeapon(void)
{
    ...
    if (VM_OnEvent(EVENT_DISPLAYWEAPON, pPlayer->i, screenpeek) == 0)
    {
        ...
        {
            static int32_t fistPos[MAXPLAYERS];
            int const drawPlayer = clamp<int>(screenpeek, 0, MAXPLAYERS - 1);
            ...
            if (pPlayer->jetpack_on == 0)
            {
                ...
                fistPos[drawPlayer] += playerXvel >> 3;
            }

            currentWeapon = weaponX;
            weaponX += sintable[(fistPos[drawPlayer])&2047] >> 10;
            G_DrawTileScaled(weaponX + 250 - (pPlayer->look_ang >> 1), weaponY + 258 - (klabs(sintable[(fistPos[drawPlayer])&2047] >> 8)),
                FIST, weaponShade, weaponBits, weaponPal, W_FIST);
            weaponX = currentWeapon - (sintable[(fistPos[drawPlayer])&2047] >> 10);
            G_DrawTileScaled(weaponX + 40 - (pPlayer->look_ang >> 1), weaponY + 200 + (klabs(sintable[(fistPos[drawPlayer])&2047] >> 8)), FIST,
...
#endif
        {
            ...
            case RPG_WEAPON:
                ...
                G_DrawWeaponTile(weaponX + 164, (weaponY << 1) + 176 - weaponYOffset, G_GetRpgHudTile(), weaponShade, weaponBits, weaponPal, W_RPG);
                break;
...
void P_GetInput(int const playerNum)
{
    ...
    if (g_cheatBufLen > 1 || (pPlayer->gm & (MODE_MENU|MODE_TYPE)) || (ud.pause_on && !KB_KeyPressed(sc_Pause)))
    {
        if (!(pPlayer->gm&MODE_MENU))
        {
            ...
            CONTROL_SuppressJoystickInput = CONTROL_SuppressJoystickInput || suppressJoystickForSplitScreen;
    ...
    int const joystickAimWeight = G_GetJoystickAimWeightForPlayer(playerNum);
    if (joystickAimWeight)
    {
        int const absyaw = klabs(info.dyaw);
        int const abspitch = klabs(info.dpitch);
        int const aimWeightDivisor = max(1, 8 - joystickAimWeight);
        ...
        {
            if (info.dpitch > 0)
                info.dpitch = max(0, info.dpitch - tabledivide32_noinline(absyaw - abspitch, aimWeightDivisor));
            else if (info.dpitch < 0)
                info.dpitch = min(0, info.dpitch + tabledivide32_noinline(absyaw - abspitch, aimWeightDivisor));
        ...
        {
            if (info.dyaw > 0)
                info.dyaw = max(0, info.dyaw - tabledivide32_noinline(abspitch - absyaw, aimWeightDivisor));
            else if (info.dyaw < 0)
                info.dyaw = min(0, info.dyaw + tabledivide32_noinline(abspitch - absyaw, aimWeightDivisor));
    ...
    if (pPlayer->last_pissed_time > 0)
    {
        switch (--pPlayer->last_pissed_time)
        {
            case GAMETICSPERSEC * 219:
            {
                ...
                if (G_ShouldPlayPlayerActionSound(playerNum))
...
void P_CheckWeapon(DukePlayer_t *pPlayer)
{
    ...
    {
        weaponNum = G_GetLocalPlayerWeaponChoiceSetting(playerNum, wpnInc);
    ...
    if (wpnInc > FREEZE_WEAPON)
    {
        weaponNum = KNEE_WEAPON;

        for (int fallbackWeapon = PISTOL_WEAPON; fallbackWeapon <= FREEZE_WEAPON; ++fallbackWeapon)
        {
            if ((pPlayer->gotweapon & (1 << fallbackWeapon)) && pPlayer->ammo_amount[fallbackWeapon] > 0)
            {
                weaponNum = fallbackWeapon;
                break;
            }
        }
    }
...
void P_FragPlayer(int playerNum)
{
    ...
    if (pSprite->pal != 1)
    {
        ...
        P_AddForceFeedbackForPlayer(playerNum, pPlayer->max_player_health << FF_PLAYER_DMG_SCALE,
                                    pPlayer->max_player_health << FF_PLAYER_DMG_SCALE, pPlayer->max_player_health << FF_PLAYER_TIME_SCALE);
    ...
    else
    {
        ...
        else
        {
                ...
                (pPlayer->on_ground == 0 && pPlayer->vel.z > (ACTOR_MAXFALLINGZVEL >> 1)))
            {
                ...
                P_AddForceFeedbackForPlayer(playerNum, (pPlayer->hard_landing << FF_PLAYER_DMG_SCALE),
                                            (pPlayer->hard_landing << FF_PLAYER_DMG_SCALE), (pPlayer->hard_landing << FF_PLAYER_TIME_SCALE));
            }
    ...
    if (!FURY && pPlayer->knee_incs > 0)
    {
        ...
        if (++pPlayer->knee_incs > 15)
        {
                ...
                dist(&sprite[pPlayer->i], &sprite[pPlayer->actorsqu]) < 1400)
            {
                ...
                P_AddForceFeedbackForPlayer(playerNum, (dmg << FF_WEAPON_DMG_SCALE), (dmg << FF_WEAPON_DMG_SCALE),
                                            max<int>(FF_WEAPON_MAX_TIME, dmg << FF_WEAPON_TIME_SCALE));
