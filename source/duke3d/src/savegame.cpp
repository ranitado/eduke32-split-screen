...
{
#ifdef SPLITSCREEN_MOD_HACKS
    return playerCount >= 1 && playerCount <= MAXSPLITSCREENCONTROLLERS;
#else
...
#ifdef SPLITSCREEN_MOD_HACKS
    G_SetConfiguredSplitScreenPlayerCount(max<int32_t>(G_GetConfiguredSplitScreenPlayerCount(), playerCount));
    G_SetActiveSplitScreenPlayerCount(playerCount);
#endif

    ud.multimode = playerCount;
    g_mostConcurrentPlayers = playerCount;
}
...
int32_t G_LoadPlayer(savebrief_t & sv)
{
    if (sv.isExt)
    {
        ...
        if (numplayers != ud.multimode && !G_CanUseLocalSavePlayerCount(numplayers))
        {
...

        {
            ...
            G_ApplyLocalSavePlayerCount(numplayers);
...
static int32_t G_TryReadSaveDetailsMetadata(char const * const fn, int32_t * const secrets, int32_t * const maxSecrets)
{
    ...
    BFILE * const fp = Bfopen(detailsMeta, "rb");
    if (fp == nullptr)
        return -1;
...
static void G_WriteSaveDetailsMetadata(char const * const fn)
{
    ...
    BFILE * const fp = Bfopen(detailsMeta, "wb");
    if (fp == nullptr)
        return;
