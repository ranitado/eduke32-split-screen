...
static int32_t S_GetClosestLocalListener(int32_t const spriteNum, vec3_t const &pos)
{
    if (!G_HaveSplitScreen())
        return screenpeek;

    int32_t bestPlayer = screenpeek;
    ...
    for (int viewIndex = 0; viewIndex < splitPlayerCount; ++viewIndex)
    {
        ...
        if ((unsigned)spriteNum < MAXSPRITES && pPlayer->i == spriteNum)
            return playerNum;

        int32_t const dist = FindDistance3D(pPlayer->pos.x - pos.x, pPlayer->pos.y - pos.y, pPlayer->pos.z - pos.z);
        if (dist >= bestDist)
            continue;
...
static bool S_IsLocalSplitScreenPlayerSprite(int32_t const spriteNum)
{
    if (!G_HaveSplitScreen() || (unsigned)spriteNum >= MAXSPRITES || PN(spriteNum) != APLAYER)
        return false;
    ...
    for (int viewIndex = 0; viewIndex < splitPlayerCount; ++viewIndex)
    {
        ...
        if (g_player[playerNum].ps->i == spriteNum)
            return true;
    }
...
static int S_CalcDistAndAng(int spriteNum, int soundNum, int sectNum, int angle,
                             ...
                             int *listenerPlayerPtr = nullptr)
{
    int sndang = 0, sndist = 0, explosion = 0;
    int listenerPlayer = screenpeek;
    int listenerSect = sectNum;
    int listenerAngle = angle;
    vec3_t listenerCam = cam;

#ifdef SPLITSCREEN_MOD_HACKS
    listenerPlayer = S_GetClosestLocalListener(spriteNum, pos);
    if ((unsigned)listenerPlayer < MAXPLAYERS && g_player[listenerPlayer].ps != nullptr)
    {
        auto const pListener = g_player[listenerPlayer].ps;
        listenerCam   = pListener->pos;
        listenerSect  = pListener->cursectnum;
        listenerAngle = fix16_to_int(pListener->q16ang);
    }
#endif

    if (listenerPlayerPtr != nullptr)
        *listenerPlayerPtr = listenerPlayer;

    if (PN(spriteNum) == APLAYER && P_Get(spriteNum) == listenerPlayer)
        goto sound_further_processing;

    sndang = S_GetAngle(listenerAngle, listenerCam, pos);
    sndist = FindDistance3D(listenerCam.x-pos.x, listenerCam.y-pos.y, (listenerCam.z-pos.z));
...
#ifndef EDUKE32_STANDALONE
    if (!FURY && ((unsigned)listenerSect >= (unsigned)numsectors || (unsigned)SECT(spriteNum) >= (unsigned)numsectors
        || (sndist && PN(spriteNum) != MUSICANDSFX
            && (!sectorsareconnected(listenerSect, SECT(spriteNum))
                || !cansee(listenerCam.x, listenerCam.y, listenerCam.z - (24 << 8), listenerSect, SX(spriteNum), SY(spriteNum), SZ(spriteNum) - (24 << 8), SECT(spriteNum))))))
        sndist += sndist>>5;
#else
    UNREFERENCED_PARAMETER(listenerSect);
#endif
...
static int S_PlaySoundResolved(int sndnum)
{
    ...
    fix16_t const volume = fix16_from_float(((snd->flags & SF_TALK) || ((snd->flags & SF_SPEECH) && ud.config.VoiceVolume > ud.config.FXVolume))
    ...
    int const voice = (snd->flags & SF_LOOP)
                        ? FX_Play(snd->ptr, snd->len, 0, -1, pitch, LOUDESTVOLUME, LOUDESTVOLUME,
                                  LOUDESTVOLUME, snd->len, volume, (sndnum * MAXSOUNDINSTANCES) + sndSlot)
...
int S_PlaySound3D(int num, int spriteNum, const vec3_t& pos)
{
    ...
    if (snd->flags & SF_TALK)
    {
        if ((g_netServer || ud.multimode > 1) && PN(spriteNum) == APLAYER && P_Get(spriteNum) != screenpeek
            && !S_IsLocalSplitScreenPlayerSprite(spriteNum)) // other player sound
        {
    ...
    int listenerPlayer = screenpeek;
    int const explosionp = S_CalcDistAndAng(spriteNum, sndNum, CAMERA(sect), fix16_to_int(CAMERA(q16ang)), CAMERA(pos), pos,
                                            &sndist, &sndang, &listenerPlayer);

    int pitch = S_GetPitch(sndNum);
    auto pOther = ((unsigned)listenerPlayer < MAXPLAYERS && g_player[listenerPlayer].ps != nullptr)
                    ? g_player[listenerPlayer].ps
                    : g_player[screenpeek].ps;
