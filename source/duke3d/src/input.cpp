...
static bool I_ShouldSuppressJoystickMenuInput(void)
{
    return ud.config.SplitScreenSeparateKeyboardMouse != 0;
}
...
static UserInput *I_GetUserInput(void)
{
    ...
    CONTROL_SuppressJoystickInput = CONTROL_SuppressJoystickInput || I_ShouldSuppressJoystickMenuInput();
    UserInput * const info = CONTROL_GetUserInput(nullptr);
...
{
    return KB_KeyWaiting() || MOUSE_GetButtons() || (!I_ShouldSuppressJoystickMenuInput() && JOYSTICK_GetButtons())
...
{
    return I_GetUserInput()->dir == dir_Up || (CONTROL_LastSeenInput != LastSeenInput::Joystick && BUTTON(gamefunc_Move_Forward));
}

int32_t I_MenuDown(void)
{
    return I_GetUserInput()->dir == dir_Down || (CONTROL_LastSeenInput != LastSeenInput::Joystick && BUTTON(gamefunc_Move_Backward));
}

int32_t I_MenuLeft(void) { return I_GetUserInput()->dir == dir_Left || (CONTROL_LastSeenInput != LastSeenInput::Joystick && (BUTTON(gamefunc_Turn_Left) || BUTTON(gamefunc_Strafe_Left))); }
int32_t I_MenuRight(void) { return I_GetUserInput()->dir == dir_Right || (CONTROL_LastSeenInput != LastSeenInput::Joystick && (BUTTON(gamefunc_Turn_Right) || BUTTON(gamefunc_Strafe_Right))); }
