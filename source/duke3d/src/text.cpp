...
static int32_t constexpr QUOTE_NOTIFICATION_Y_NUDGE_SINGLE = 6 << 16;
static int32_t constexpr QUOTE_NOTIFICATION_Y_NUDGE_SPLIT  = 14 << 16;

static FORCE_INLINE void G_GetTextClipBounds(int32_t *x1, int32_t *y1, int32_t *x2, int32_t *y2)
{
...
int32_t minitext_(int32_t x, int32_t y, const char *t, int32_t s, int32_t p, int32_t sb)
{
    ...
    dim = G_ScreenText(MF_Minifont.tilenum, x, y, z, 0, 0, t, s, p, sb, 0, MF_Minifont.emptychar.x, MF_Minifont.emptychar.y, MF_Minifont.between.x, MF_Minifont.between.y, MF_Minifont.textflags, x1, y1, x2, y2);
...
void menutext_(int32_t x, int32_t y, int32_t s, char const *t, int32_t o, int32_t f)
{
    int32_t x1, y1, x2, y2;
    G_GetTextClipBounds(&x1, &y1, &x2, &y2);
    G_ScreenText(MF_Redfont.tilenum, x, y - (12<<16), MF_Redfont.zoom, 0, 0, t, s, MF_Redfont.pal, o, 0, MF_Redfont.emptychar.x, MF_Redfont.emptychar.y, MF_Redfont.between.x, MF_Redfont.between.y, f|MF_Redfont.textflags|TEXT_LITERALESCAPE, x1, y1, x2, y2);
}
...
void captionmenutext(int32_t x, int32_t y, char const *t)
{
    ...
    G_ScreenText(MF_Redfont.tilenum, x, y, MF_Redfont.zoom, 0, 0, t, 0, ud.menutitle_pal, g_textstat, 0,
                 ...
                 MF_Redfont.textflags|TEXT_LITERALESCAPE|TEXT_XCENTER|TEXT_YCENTER|TEXT_YOFFSETZERO, x1, y1, x2, y2);
}
...
static FORCE_INLINE int32_t G_GetQuoteTextStat(int32_t const orientation)
{
    int32_t textStat = orientation | g_textstat;

    if (G_HaveSplitScreen() && G_GetSplitScreenPlayerCount() > 1)
        textStat &= ~RS_NOCLIP;

    return textStat;
}

static vec2_t G_DrawQuoteText(int32_t x, int32_t y, const char *t, int32_t zoom, int32_t shade, int32_t pal, int32_t orientation,
                              int32_t alpha, int32_t xSpace, int32_t ySpace, int32_t xBetween, int32_t yBetween, int32_t flags)
{
    int32_t x1 = 0, y1 = 0, x2 = xdim - 1, y2 = ydim - 1;
    ...
    return G_ScreenText(MF_Bluefont.tilenum, x, y, zoom, 0, 0, t, shade, pal, G_GetQuoteTextStat(orientation), alpha,
                        xSpace, ySpace, xBetween, yBetween, flags, x1, y1, x2, y2);
}

static FORCE_INLINE int32_t G_GetViewportQuoteTextStat(int32_t const orientation)
{
    int32_t textStat = orientation | g_textstat | ROTATESPRITE_FULL16 | RS_TOPLEFT;
    textStat &= ~(RS_AUTO | RS_NOCLIP);
    return textStat;
}

static vec2_t G_DrawQuoteTextInBounds(int32_t x, int32_t y, const char *t, int32_t zoom, int32_t shade, int32_t pal,
                                      int32_t orientation, int32_t alpha, int32_t xSpace, int32_t ySpace,
                                      int32_t xBetween, int32_t yBetween, int32_t flags,
                                      int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
    return G_ScreenText(MF_Bluefont.tilenum, x, y, zoom, 0, 0, t, shade, pal, G_GetViewportQuoteTextStat(orientation), alpha,
                        xSpace, ySpace, xBetween, yBetween, flags, x1, y1, x2, y2);
}
...
void G_PrintGameQuotes(int32_t snum)
{
    ...
    bool const haveSplitScreen = G_HaveSplitScreen() && G_GetSplitScreenPlayerCount() > 1;
    int32_t const yNudge = haveSplitScreen ? QUOTE_NOTIFICATION_Y_NUDGE_SPLIT : QUOTE_NOTIFICATION_Y_NUDGE_SINGLE;
    int32_t const ybase = (fragbarheight()<<16) + text_ypos() + ud.quote_yoffset + yNudge;
    int32_t height = 0;
    int32_t k = ps->fta;
    ...
    do
    {
        ...
        int32_t y = ybase;
        if (reserved_quote)
            y = haveSplitScreen ? (70 << 16) : (140 << 16);

        height = G_DrawQuoteText(160 << 16, y, apStrings[ps->ftq], MF_Bluefont.zoom, textsh(k), 0, texto(k), texta(k),
                                 MF_Bluefont.emptychar.x, MF_Bluefont.emptychar.y, MF_Bluefont.between.x, MF_Bluefont.between.y,
                                 MF_Bluefont.textflags | TEXT_XCENTER).y + (1<<16);
    }
    while (0);
    ...
    for (int i = 0; i < MAXUSERQUOTES; i++)
    {
        ...
        height = G_DrawQuoteText(mpgametext_x, y, user_quote[i], textsc(MF_Bluefont.zoom), textsh(k), MF_Bluefont.pal,
                                 texto(k), texta(k),
                                 MF_Bluefont.emptychar.x, MF_Bluefont.emptychar.y, MF_Bluefont.between.x, MF_Bluefont.between.y,
                                 MF_Bluefont.textflags).y + textsc(1<<16);
        y += k <= 4 ? (height * (k-1))>>2 : height;
    }
}

void G_PrintGameQuotesInViewport(int32_t snum, int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
    if ((unsigned)snum >= MAXPLAYERS || g_player[snum].ps == nullptr || x2 < x1 || y2 < y1)
        return;

    auto const ps = g_player[snum].ps;
    int32_t const width  = x2 - x1 + 1;
    int32_t const height = y2 - y1 + 1;

    if (width <= 0 || height <= 0)
        return;

    int32_t const viewportScale = min<int32_t>(divscale16(width, 320), divscale16(height, 200));
    int32_t const yScale        = divscale16(height, 200);
    int32_t const ybase         = (y1 << 16) + mulscale16((fragbarheight() << 16) + text_ypos() + ud.quote_yoffset, yScale) + QUOTE_NOTIFICATION_Y_NUDGE_SPLIT;
    int32_t const xcenter       = (x1 + x2 + 1) << 15;
    int32_t const userX         = (x1 << 16) + mulscale16(mpgametext_x, viewportScale);
    int32_t const primaryZoom   = mulscale16(MF_Bluefont.zoom, viewportScale);
    int32_t const userZoom      = mulscale16(textsc(MF_Bluefont.zoom), viewportScale);
    int32_t const reservedQuote = (ps->ftq >= QUOTE_RESERVED && ps->ftq <= QUOTE_RESERVED3);
    int32_t quoteHeight = 0;
    int32_t k = ps->fta;
    ...
    do
    {
        ...
        int32_t y = ybase;
        if (reservedQuote)
            y = (y1 << 16) + mulscale16(70 << 16, yScale);

        quoteHeight = G_DrawQuoteTextInBounds(xcenter, y, apStrings[ps->ftq], primaryZoom, textsh(k), 0, texto(k), texta(k),
                                              MF_Bluefont.emptychar.x, MF_Bluefont.emptychar.y,
                                              MF_Bluefont.between.x, MF_Bluefont.between.y,
                                              MF_Bluefont.textflags | TEXT_XCENTER, x1, y1, x2, y2).y
                    + mulscale16(1 << 16, viewportScale);
    }
    while (0);
    ...
    for (int i = 0; i < MAXUSERQUOTES; i++)
    {
        ...
        quoteHeight = G_DrawQuoteTextInBounds(userX, y, user_quote[i], userZoom, textsh(k), MF_Bluefont.pal,
                                              0, texta(k),
                                              MF_Bluefont.emptychar.x, MF_Bluefont.emptychar.y,
                                              MF_Bluefont.between.x, MF_Bluefont.between.y,
                                              MF_Bluefont.textflags, x1, y1, x2, y2).y
                    + mulscale16(textsc(1 << 16), viewportScale);
        y += quoteHeight;
    }
}
