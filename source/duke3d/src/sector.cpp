...
{
    return playerNum == screenpeek || G_HaveSplitScreen() || (g_gametypeFlags[ud.coop] & GAMETYPE_COOPSOUND);
}
...
void P_CheckSectors(int playerNum)
{
    ...
    else if (!pPlayer->toggle_key_flag)
    {
        ...
        if (foundWall >= 0 && wallDist < 1280 && wall[foundWall].overpicnum == MIRROR)
            if (wall[foundWall].lotag > 0 && !A_CheckSoundPlaying(pPlayer->i,wall[foundWall].lotag) && G_ShouldPlayPlayerActionSound(playerNum))
            {
