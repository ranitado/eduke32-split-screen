...
static FORCE_INLINE int32_t G_ClassicWeaponStayEnabled(void)
{
...
ACTOR_STATIC void G_MoveActors(void)
{
    ...
    while (spriteNum >= 0)
    {
        ...
        case HEAVYHBOMB__:
        {
            ...
            int32_t playerDist;
            playerNum = G_FindPlayerForLocalPickup(pSprite, &playerDist);
            pPlayer   = g_player[playerNum].ps;
...
DETONATEB:
            ...
            else if (pSprite->picnum == HEAVYHBOMB && playerDist < 788 && pData[0] > 7 && pSprite->xvel == 0)
            {
                           ...
                           pPlayer->pos.x, pPlayer->pos.y, pPlayer->pos.z, pPlayer->cursectnum))
                {
                    ...
                    {
                        int const keepForOtherLocalPlayers = G_HandleLocalPerPlayerPickup(playerNum, spriteNum);

                        if (G_ClassicWeaponStayEnabled() && pSprite->owner == spriteNum)
                        {
                        ...
                        if (pSprite->owner != spriteNum || ud.respawn_items == 0)
                        {
                            if (keepForOtherLocalPlayers || (pSprite->owner == spriteNum && G_ClassicWeaponStayEnabled()))
                                goto next_sprite;
                            DELETE_SPRITE_AND_CONTINUE(spriteNum);
                        }
                        else
