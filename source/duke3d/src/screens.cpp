...
int32_t g_noLogo = 0;
int32_t g_startupEscapeExitsGame = 0;

void G_QuitIfStartupEscapePressed(void)
{
    if (g_startupEscapeExitsGame && KB_KeyPressed(sc_Escape))
        G_GameExit();
}

static void G_DrawStartupVersionText(void)
{
    minitext(1, 193, "V0.6", 0, 2 + 8 + 16 + 128);
}
...
static void G_DrawCrosshairForPlayer(int32_t const playerNum)
{
    auto const pPlayer = g_player[playerNum].ps;
    if (pPlayer == nullptr || pPlayer->newowner != -1 || ud.overhead_on != 0 || !ud.crosshair || ud.camerasprite != -1)
        return;

    ud.returnvar[0] = (160 << 16) - (pPlayer->look_ang << 15);
    ud.returnvar[1] = 100 << 16;

    int32_t const crosshairTile = VM_OnEventWithReturn(EVENT_DISPLAYCROSSHAIR, pPlayer->i, playerNum, CROSSHAIR);
    if ((unsigned)crosshairTile >= MAXTILES)
        return;

    vec2_t crosshairPos = { ud.returnvar[0], ud.returnvar[1] };
    uint8_t crosshairPal = CROSSHAIR_PAL;
    uint32_t crosshairBits = 1 | 2;
    uint32_t crosshairScale = divscale16(ud.crosshairscale, 100);

    auto const oldAspect = yxaspect;
    if (FURY)
    {
        ...
        crosshairScale = scale(crosshairScale, ydim << 2, xdim * 3) >> 1;
        crosshairPal = 0;
        crosshairBits |= 1024;
        renderSetAspect(viewingrange, 65536);
    }

    rotatesprite_win(crosshairPos.x, crosshairPos.y, crosshairScale, 0, crosshairTile, 0, crosshairPal, crosshairBits);

    if (FURY)
        renderSetAspect(viewingrange, oldAspect);
}

static void G_DrawSplitScreenQuote(int32_t const playerNum, splitscreen_viewport_t const &viewport)
{
    G_PrintGameQuotesInViewport(playerNum, viewport.x, viewport.y, viewport.x + viewport.width - 1, viewport.y + viewport.height - 1);
}
...
static void G_DrawSplitScreenTintOverlay(int32_t const playerNum, splitscreen_viewport_t const &viewport)
{
    ...
    int r = clamp(tabledivide32_noinline(tint.r, tint.sumf) << 2, 0, 255);
    int g = clamp(tabledivide32_noinline(tint.g, tint.sumf) << 2, 0, 255);
    int b = clamp(tabledivide32_noinline(tint.b, tint.sumf) << 2, 0, 255);
    ...
    palette_t overlay = { (uint8_t) r, (uint8_t) g, (uint8_t) b, (uint8_t) paletteGetClosestColor(r, g, b) };
    int const x1 = viewport.x << 12;
    int const x2 = (viewport.x + viewport.width - 1) << 12;
    int const y1 = viewport.y;
    int const y2 = viewport.y + viewport.height;

    for (int y = y1; y < y2; y += 2)
        drawlinergb(x1, y << 12, x2, y << 12, overlay);
}

static int32_t G_ScaleSplitScreenHudCoord(int32_t const origin, int32_t const coord, int32_t const scale)
{
...
static void G_DrawSplitScreenHudNumber(int32_t const x, int32_t const y, int32_t const value, int32_t const shade,
                                       int32_t const pal, int32_t const x1, int32_t const y1,
                                       int32_t const x2, int32_t const y2, int32_t const scale)
{
    char numberText[12];
    Bsprintf(numberText, "%d", value);

    // Keep this out of rotatesprite_win's window-relative path; the box artwork is drawn in absolute viewport pixels too.
    int32_t const clipX2 = x2 > x1 ? x2 - 1 : x2;
    G_ScreenText(DIGITALNUM, x, y, scale, 0, 0, numberText, shade, pal,
                 RS_TOPLEFT | RS_ALIGN_L | ROTATESPRITE_FULL16, 0,
                 4 << 16, 8 << 16, 1 << 16, 0,
...
static void G_DrawSplitScreenOverlayForPlayer(int32_t const playerNum)
{
    ...
    int viewIndex = -1;
    int const splitPlayerCount = G_GetSplitScreenPlayerCount();
    for (int i = 0; i < splitPlayerCount; ++i)
    {
        if (G_GetSplitScreenPlayer(i) == playerNum)
        {
            viewIndex = i;
            break;
        }
    ...
    if (viewIndex < 0 || G_GetSplitScreenViewport(viewIndex, &viewport) != 0 || viewport.width <= 0 || viewport.height <= 0)
        return;

    int const savedPeek = screenpeek;

    screenpeek = playerNum;
    videoSetViewableArea(viewport.x, viewport.y, viewport.x + viewport.width - 1, viewport.y + viewport.height - 1);

    auto const pPlayer = g_player[playerNum].ps;

    if (ud.camerasprite == -1 && ud.overhead_on != 2)
    {
        if (pPlayer->newowner >= 0)
            G_DrawCameraText(pPlayer->newowner);
        else
        {
            P_DisplayWeapon();

            if (pPlayer->over_shoulder_on == 0)
                P_DisplayScuba();
        }
    }

    if (pPlayer->invdisptime > 0)
        G_DrawInventory(pPlayer);
    ...
    videoSetViewableArea(viewport.x, viewport.y, viewport.x + viewport.width - 1, viewport.y + viewport.height - 1);
    G_DrawSplitScreenQuote(playerNum, viewport);
    G_DrawCrosshairForPlayer(playerNum);
    G_DrawSplitScreenTintOverlay(playerNum, viewport);

    screenpeek = savedPeek;
}
...
void G_DisplayRest(int32_t smoothratio)
{
    ...
    bool const haveSplitScreen = G_HaveSplitScreen() != 0;
#ifdef SPLITSCREEN_MOD_HACKS
    bool const legacySharedSplitTint = !haveSplitScreen;
    auto const pp2View = legacySharedSplitTint ? G_GetSplitScreenPlayer(1) : -1;
    auto const pp2 = pp2View >= 0 ? g_player[pp2View].ps : NULL;
#endif
    ...
    if (g_restorePalette)
    {
        ...
        if (g_restorePalette < 2 || omovethingscnt+1 == g_moveThingsCount)
        {
            ...
            if (pp2)  // splitscreen HACK: BASEPAL trumps all, then it's arbitrary.
                pal = min<int32_t>(pal, pp2->palette);
#endif
    ...
    if (haveSplitScreen)
    {
        ...
        for (int viewIndex = 0; viewIndex < splitPlayerCount; ++viewIndex)
        {
            int const playerNum = G_GetSplitScreenPlayer(viewIndex);
            G_DrawSplitScreenOverlayForPlayer(playerNum);
        }
    ...
    else
    {
        ...
        G_DrawCrosshairForPlayer(screenpeek);
    }

    if (ud.show_level_text && hud_showmapname && g_levelTextTime > 1 && haveSplitScreen)
    {
        int32_t o = g_textstat;
...
static void G_DisplayMPResultsScreen(void)
{
    ...
    if (PLUTOPAK)   // JBF 20030804
    {
        ...
        rotatesprite_fs(260<<16, 36<<16, logoZoom, 0, logoTile, 0, 0, 2+8);
    }
...
static int32_t G_GetBonusPlayerCount(void)
{
    return G_HaveSplitScreen() ? max<int32_t>(G_GetSplitScreenPlayerCount(), 1) : 1;
}

static int32_t G_GetBonusPlayer(int32_t const playerIndex)
{
    if (!G_HaveSplitScreen())
        return myconnectindex;

    int32_t const playerNum = G_GetSplitScreenPlayer(playerIndex);
    return ((unsigned)playerNum < MAXPLAYERS && g_player[playerNum].ps != nullptr) ? playerNum : myconnectindex;
}

static int32_t G_GetBonusActorsKilled(void)
{
    if (!G_HaveSplitScreen())
        return g_player[myconnectindex].ps->actors_killed;
...
static int32_t G_GetBonusMaxActorsKilled(void)
{
    int32_t maxActorsKilled = g_player[myconnectindex].ps->max_actors_killed;

    if (!G_HaveSplitScreen())
        return maxActorsKilled;
    ...
    for (int32_t playerIndex = 0, playerCount = G_GetBonusPlayerCount(); playerIndex < playerCount; ++playerIndex)
    {
        ...
        if (pPlayer != nullptr)
            maxActorsKilled = max<int32_t>(maxActorsKilled, pPlayer->max_actors_killed);
    }

    return maxActorsKilled;
}

static int32_t G_GetBonusSecretRooms(void)
{
    if (!G_HaveSplitScreen())
...
static int32_t G_GetBonusMaxSecretRooms(void)
{
    int32_t maxSecretRooms = g_player[myconnectindex].ps->max_secret_rooms;
...
void G_BonusScreen(int32_t bonusonly)
{
    ...
    if (bonusonly || g_netServer || (ud.multimode > 1 && !G_HaveSplitScreen())) return;
