...
#define rotatesprite_fs G_RotateSpriteHud
...
{
    return (200<<16) - sbarsc(200<<16) + sbarsc(y) + G_GetSplitScreenHudYOffset();
}
...
static inline void G_DrawDigiNum_(int32_t x, int32_t yofs, int32_t y, int32_t n, char s, int32_t cs)
{
    ...
    G_DrawTXDigiNumZ(DIGITALNUM, sbarx16(x), yofs + sbary16(y), n, s, 0, cs|ROTATESPRITE_FULL16, x1, y1, x2, y2, sbarsc(65536L));
}
...
static int32_t G_GetSplitScreenHudXOffset(void)
{
    ...
    // Status bar sprites use RS_NOCLIP, so they are positioned in full-screen
...
static int32_t G_GetSplitScreenHudYOffset(void)
{
    ...
    return scale(viewport.y + viewport.height - ydim, 200, ydim) << 16;
}

static int32_t G_GetSplitScreenInventoryYOffset(void)
{
    splitscreen_viewport_t viewport {};
    int const playerNum = g_sbarPlayer >= 0 ? g_sbarPlayer : screenpeek;
    if (G_GetSplitScreenViewportForPlayer(playerNum, &viewport) != 0)
    ...
    return viewport.width > viewport.height * 2 ? -(20 << 16) : 0;
}
...
void G_DrawInventory(const DukePlayer_t *p)
{
    ...
    y += G_GetSplitScreenInventoryYOffset();
...
static inline void rotatesprite_althud(int32_t sx, int32_t sy, int32_t z, int16_t a, int16_t picnum, int8_t dashade, char dapalnum, int32_t dastat)
{
    ...
    rotatesprite_(sbarx(sx), sbary(sy), z, a, picnum, dashade, dapalnum, dastat, 0, 0, x1, y1, x2, y2);
}
...
static inline void rotatesprite_althudr(int32_t sx, int32_t sy, int32_t z, int16_t a, int16_t picnum, int8_t dashade, char dapalnum, int32_t dastat)
{
    ...
    rotatesprite_(sbarxr(sx), sbary(sy), z, a, picnum, dashade, dapalnum, dastat, 0, 0, x1, y1, x2, y2);
}
...
void G_DrawStatusBar(int32_t snum)
{
    ...
    if (!G_HaveSplitScreen() && (g_netServer || ud.multimode > 1) && ((g_gametypeFlags[ud.coop] & GAMETYPE_FRAGBAR)))
    {
        if (pus)
            G_DrawFrags();
        else
