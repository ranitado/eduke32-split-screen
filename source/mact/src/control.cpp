...
bool CONTROL_SuppressJoystickInput = false;
...
static void controlClearButtonStates(int32_t NumButtons, ControlButtonState_t *const b)
{
...
static void controlUpdateButtonStates(void)
{
    ...
    if (CONTROL_SuppressJoystickInput)
        controlClearButtonStates(CONTROL_NumJoyButtons, joyButtons);
...
static void controlClearJoystickAxisStates(void)
{
    for (int i=CONTROL_NumJoyAxes-1; i>=0; i--)
    {
        joyAxes[i].last = joyAxes[i].axis;
        joyAxes[i].axis = {};
    }
}
...
static void controlPollDevices(ControlInfo *const info)
{
    ...
    if (CONTROL_SuppressJoystickInput)
        controlClearJoystickAxisStates();
    else if (CONTROL_JoystickEnabled)
    {
...
UserInput *CONTROL_GetUserInput(UserInput *info)
{
    ...
    uint32_t const controllerButtons = CONTROL_SuppressJoystickInput ? 0 : JOYSTICK_GetControllerButtons();

    if (!CONTROL_SuppressJoystickInput && ((joyAxes[CONTROLLER_AXIS_LEFTY].axis.digital == -1 && SCALEAXIS(LEFTY) <= -SATU(LEFTY))
        || (controllerButtons & (1 << CONTROLLER_BUTTON_DPAD_UP))))
        newdir = dir_Up;
    else if (!CONTROL_SuppressJoystickInput && ((joyAxes[CONTROLLER_AXIS_LEFTY].axis.digital == 1 && SCALEAXIS(LEFTY) >= SATU(LEFTY))
                || (controllerButtons & (1 << CONTROLLER_BUTTON_DPAD_DOWN))))
        newdir = dir_Down;
    else if (!CONTROL_SuppressJoystickInput && ((joyAxes[CONTROLLER_AXIS_LEFTX].axis.digital == -1 && SCALEAXIS(LEFTX) <= -SATU(LEFTX))
                || (controllerButtons & (1 << CONTROLLER_BUTTON_DPAD_LEFT))))
        newdir = dir_Left;
    else if (!CONTROL_SuppressJoystickInput && ((joyAxes[CONTROLLER_AXIS_LEFTX].axis.digital == 1 && SCALEAXIS(LEFTX) >= SATU(LEFTX))
                || (controllerButtons & (1 << CONTROLLER_BUTTON_DPAD_RIGHT))))
        newdir = dir_Right;
    ...
    info->b_advance = KB_KeyPressed(sc_Enter) || KB_KeyPressed(sc_kpad_Enter) || (MOUSE_GetButtons() & M_LEFTBUTTON)
                    || (controllerButtons & (1 << CONTROLLER_BUTTON_A));
    info->b_return   = KB_KeyPressed(sc_Escape) || (MOUSE_GetButtons() & M_RIGHTBUTTON) || (controllerButtons & (1 << CONTROLLER_BUTTON_B));
    info->b_escape = KB_KeyPressed(sc_Escape) || (controllerButtons & (1 << CONTROLLER_BUTTON_START));
