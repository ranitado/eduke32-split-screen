...
vec2_t screentextRender(ScreenText_t const & data)
{
    ...
    bool const useWindowRelativeClip = data.b1.x == windowxy1.x && data.b1.y == windowxy1.y
                                    && data.b2.x == windowxy2.x && data.b2.y == windowxy2.y;
    ...
    while (text < end && (glyph = *text))
    {
        ...
        else
        {
            ...
            if (useWindowRelativeClip)
                rotatesprite_win_alpha(location.x, location.y, data.zoom, angle, tile, data.shade, pal, o, alpha, blendidx);
            else
                rotatesprite_(location.x, location.y, data.zoom, angle, tile, data.shade, pal, o, alpha, blendidx,
                              data.b1.x, data.b1.y, data.b2.x, data.b2.y);
