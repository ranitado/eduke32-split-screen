...
static constexpr int MAX_LOCAL_GAMEPADS = 4;
...
static uint16_t gameControllerRumbleLow[MAX_LOCAL_GAMEPADS];
...
# endif
        ...
        case SDL_CONTROLLERAXISMOTION:
            ...
            if (appactive && ev->caxis.axis < joystick.numAxes)
                joystick.pAxis[ev->caxis.axis] = ev->caxis.value;
            break;
#endif
        ...
        case SDL_CONTROLLERBUTTONUP:
            ...
            if (appactive && ev->cbutton.button < joystick.numButtons)
            {
                if (ev->cbutton.state == SDL_PRESSED)
                    joystick.bits |= 1 << ev->cbutton.button;
                else
                    joystick.bits &= ~(1 << ev->cbutton.button);
            }
            break;
...
#if SDL_MAJOR_VERSION >= 2
        auto dorumbleGamepad = [](int32_t const gamepadIndex, uint16_t const low, uint16_t const high, uint32_t const time)
        {
            if ((unsigned)gamepadIndex < (unsigned)numGameControllers && gameControllers[gamepadIndex] != nullptr)
                SDL_GameControllerRumble(gameControllers[gamepadIndex], low, high, time);
        };

        for (int32_t gamepadIndex = 0; gamepadIndex < numGameControllers; ++gamepadIndex)
        {
            if (gameControllerRumbleLow[gamepadIndex] || gameControllerRumbleHigh[gamepadIndex])
            {
                gameControllerRumbleZeroTime[gamepadIndex] = timerGetTicks() + gameControllerRumbleTime[gamepadIndex];
                dorumbleGamepad(gamepadIndex, gameControllerRumbleLow[gamepadIndex], gameControllerRumbleHigh[gamepadIndex],
                                gameControllerRumbleTime[gamepadIndex]);
                gameControllerRumbleLow[gamepadIndex] = gameControllerRumbleHigh[gamepadIndex] = gameControllerRumbleTime[gamepadIndex] = 0;
            }
            else if (gameControllerRumbleZeroTime[gamepadIndex] && timerGetTicks() >= gameControllerRumbleZeroTime[gamepadIndex])
            {
                gameControllerRumbleZeroTime[gamepadIndex] = 0;
                dorumbleGamepad(gamepadIndex, 0, 0, 0);
            }
        }
#endif
    }
...
void joyAddGamepadRumble(int32_t const gamepadIndex, uint16_t const low, uint16_t const high, uint16_t const time)
{
#if SDL_MAJOR_VERSION >= 2 && SDL_VERSION_ATLEAST(2, 0, 9)
    if ((unsigned)gamepadIndex >= (unsigned)MAX_LOCAL_GAMEPADS || gameControllers[gamepadIndex] == nullptr)
        return;

    gameControllerRumbleLow[gamepadIndex]  = min<int>(UINT16_MAX, gameControllerRumbleLow[gamepadIndex] + low);
    gameControllerRumbleHigh[gamepadIndex] = min<int>(UINT16_MAX, gameControllerRumbleHigh[gamepadIndex] + high);
    gameControllerRumbleTime[gamepadIndex] = max<int>(time, gameControllerRumbleTime[gamepadIndex]);
#else
    UNREFERENCED_PARAMETER(gamepadIndex);
...
int32_t joyGetGamepadState(int32_t const gamepadIndex, gamepadstate_t * const state)
{
    if (state == nullptr)
        return -1;

    Bmemset(state, 0, sizeof(*state));

#if SDL_MAJOR_VERSION >= 2
    if ((unsigned)gamepadIndex >= (unsigned)numGameControllers)
        return -1;

    auto *const gameController = gameControllers[gamepadIndex];
    ...
    for (int axis = 0; axis < GAMEPAD_AXIS_COUNT; ++axis)
        state->axes[axis] = SDL_GameControllerGetAxis(gameController, (SDL_GameControllerAxis)axis);

    int const buttonCount = min<int>(SDL_CONTROLLER_BUTTON_MAX, 32);
    for (int button = 0; button < buttonCount; ++button)
    {
        if (SDL_GameControllerGetButton(gameController, (SDL_GameControllerButton)button))
